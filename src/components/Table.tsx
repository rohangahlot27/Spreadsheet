...
const Table = ({ data: initialData }: Props) => {
  ...
  const updateData = (rowIndex: number, columnId: string, value: string | boolean) => {
    setData((old) =>
      old.map((row, index) => {
  ...
  const handleAddRow = () => {
    ...
    customColumns.forEach((col) => {
      (newRow as any)[col.accessorKey] = col.type === "checkbox" ? false : "";
    });
    setData([...data, newRow]);
  };
  ...
  const handleAddColumn = () => {
    ...
    const type = prompt("Enter type (text, checkbox, date, number, select):") as CustomColumn["type"];
    if (!["text", "checkbox", "date", "number", "select"].includes(type)) return;

    const key = header.toLowerCase().replace(/\s+/g, "_") + "_" + nanoid(4);
    const options = type === "select"
      ? prompt("Comma-separated options")?.split(",").map((o) => o.trim()) ?? []
      : undefined;
    ...
    setData(updatedData);
    setCustomColumns([...customColumns, { accessorKey: key, header, type, options }]);
  };
  ...
  const staticColumns: ColumnDef<RowData>[] = [
    {
      ...
      cell: ({ row }) => <div className="text-center text-gray-400">{row.index + 1}</div>,
    },
    {
      id: "deleteRow",
      header: "🗑",
      ...
      enableResizing: false,
      cell: ({ row }) => (
        <button onClick={() => handleDeleteRow(row.index)} className="text-red-500 hover:text-red-700">🗑</button>
      ),
    },
    ...staticKeys.map((key) => ({
      accessorKey: key,
      header: () => (
        <div style={{ backgroundColor: headerBackgrounds[key] || undefined }} className="w-full h-full px-2 py-1">
          {key === "value"
            ? "Est. Value"
  ...
  column: ReturnType<typeof table.getAllColumns>[number];
        ...
        const isSelected = selectedCell?.row === row.index && selectedCell?.col === column.id;
          ...
          <div
            ...
            onKeyDown={(e) => {
              ...
              if (e.key === "ArrowRight" && currentIndex < headers.length - 1) {
                setSelectedCell({ row: row.index, col: headers[currentIndex + 1] });
            ...
            className={`w-full h-full px-3 py-2 cursor-pointer outline-none group ${
              isSelected ? "bg-white ring-2 ring-blue-500" : "group-hover:border-blue-400"
            }`}
          >
  ...
  const dynamicColumns: ColumnDef<RowData>[] = customColumns.map((col) => ({
    ...
    header: () => (
      <div className="flex items-center justify-between gap-2" style={{ backgroundColor: "#EEEEEE" }}>
        {col.header}
        <button onClick={() => handleDeleteColumn(col.accessorKey)} className="text-red-500 hover:text-red-700">
          ❌
        </button>
      </div>
    ),
      ...
      if (col.type === "checkbox") {
        return (
          <input
            type="checkbox"
            checked={Boolean(val)}
            onChange={(e) => updateData(row.index, col.accessorKey, e.target.checked)}
          />
        );
      }
      if (col.type === "select") {
        return (
          <select
            ...
            onChange={(e) => updateData(row.index, col.accessorKey, e.target.value)}
      ...
      <div className="overflow-x-auto bg-white border border-gray-200 rounded-md shadow-sm">
        <table className="min-w-full table-fixed border-collapse text-sm">
          <thead className="bg-gray-100 text-[#1A1A1A] font-medium sticky top-0 z-10">
           <tr>
  <th className="text-blue-600 text-lg font-bold px-3 py-2 text-left w-10 cursor-pointer" onClick={handleAddRow}>+</th>
  <th></th>
...
>
  ...
  {/* ABC with arrow */}
  <th colSpan={1} style={{ backgroundColor: "#D2E0D4" }} className="text-center px-2 py-2 border-b border-gray-300">
  ...
  <th colSpan={2} style={{ backgroundColor: "#DCCFFC" }} className="text-center px-2 py-2 border-b border-gray-300">
    <div className="flex items-center justify-center gap-1">
      <img src="/Arrow Split.svg" alt="Arrow" className="w-4 h-4" />
      Answer a Question
    </div>
  ...
  {/* Extract with arrow */}
  <th colSpan={1} style={{ backgroundColor: "#FAC2AF" }} className="text-center px-2 py-2 border-b border-gray-300">
    <div className="flex items-center justify-center gap-1">
      <img src="/Arrow Split.svg" alt="Arrow" className="w-4 h-4" />
      Extract
    </div>
  ...
  {/* New Column group */}
  <th colSpan={customColumns.length || 1} style={{ backgroundColor: "#EEEEEE" }} className="text-center px-2 py-2 border-b border-gray-300">
    New Column
  </th>

  <th className="text-green-600 px-3 py-2 text-right w-10 cursor-pointer" onClick={handleAddColumn}>+</th>
</tr>
            ...
            {table.getRowModel().rows.map((row) => (
              <tr key={row.id} className="hover:bg-gray-50 transition-colors">
                {row.getVisibleCells().map((cell) => (
                  <td key={cell.id} className="border border-gray-200 bg-white p-0">
